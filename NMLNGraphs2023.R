#making summary stats for NMLN 2023 report
...
#I added breaks(2011:2023) to the scale fill manual and then took it out and the 
#years went into the right order

#to make sure graphs go to the right place
setwd("C:/Users/User/Dropbox/WLI (2)/CASSIE/NMLN analysis/NMLNSummaryGraphs2023")
...
for (i in unique(nmln$lakename)){
 ...
 ggplot(data = onelake %>% filter(Characteristic_ID == "DO", month < 9, year < 2023))+
    ...
    theme_bw()+
    geom_point(data = onelake %>% filter(Characteristic_ID == "DO", month <10 & month > 1, year == 2023), aes(x= Result_Value, y = -1*Result_Depth_Height_Measure), fill = "lawngreen", size = 3, shape = 23)+
...
for (i in unique(nmln$lakename)){
  ...
  p0<-  ggplot(data = onelake %>% filter(Characteristic_ID == "TEMP-W", month < 9, year < 2023))+
    annotation_custom(g, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) + 
  ...
  p.5<-
   ggplot(data = onelake %>% filter(Characteristic_ID == "TEMP-W", month < 9, year < 2023))+
    annotation_custom(g, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) + 
    annotate("rect", xmin =86, xmax = 90, ymin = -Inf, ymax =0.01, alpha = .4, color = "red", fill = "red")+
...
for (i in unique(nmln.tsi$Station_ID)){
  ...
  print(ggplot(data = onelake, aes(x = year, y = TSI_mean))+
    annotate("rect", xmin = "2008" , xmax = "2023", ymin = 0, ymax = 40, alpha = .2, color = "lightskyblue", fill = "lightskyblue")+
    annotate("rect", xmin = "2008", xmax = "2023", ymin = 40, ymax = 50, alpha = .2, color = "aquamarine4", fill = "aquamarine4")+
      annotate("rect", xmin = "2008", xmax = "2023" , ymin = 50, ymax = 70, alpha = .2, color = "darkolivegreen", fill = "darkolivegreen2")+
  ...
  p =  ggplot(data = onelake, aes(x = year, y = TSI_mean))+
    annotate("rect", xmin = "2008" , xmax = "2023", ymin = 0, ymax = 40, alpha = .2, color = "lightskyblue", fill = "lightskyblue")+
    annotate("rect", xmin = "2008", xmax = "2023" , ymin = 40, ymax = 50, alpha = .2, color = "aquamarine4", fill = "aquamarine4")+
    annotate("rect", xmin = "2008", xmax = "2023" , ymin = 50, ymax = 70, alpha = .2, color = "darkolivegreen", fill = "darkolivegreen2")+
    geom_text(x = "2017", y = 45, label = "MESOTROPHIC")+
...
ggplot(data = nmln %>% filter(Characteristic_ID == "DO", month < 9, year < 2023, Station_ID == "ABBOT"))+
  annotation_custom(g, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) + 
  annotate("rect", xmin =0, xmax = 1, ymin = -Inf, ymax =0.01, alpha = .2, color = "red", fill = "red")+
  geom_text(x = .5, y = -4.2, label = "ANOXIC", aes(angle = 90,), color = "white")+
  geom_point(aes(x = Result_Value, y = -1*Result_Depth_Height_Measure, fill = Analysis_Start_Date, size = Analysis_Start_Date, shape = Analysis_Start_Date))+
  theme_bw()+
  geom_point(data = nmln %>% filter(Characteristic_ID == "DO", month <10 & month > 1, year == 2023, Station_ID == "ABBOT"), aes(x= Result_Value, y = -1*Result_Depth_Height_Measure), fill = "lawngreen", size = 3, shape = 23)+
  scale_fill_manual(values = c("gray1", "saddlebrown", "plum", "palevioletred1", "darkviolet",  "slateblue", "lightsteelblue", "mediumturquoise", "cyan4",  "deepskyblue", "mediumblue","palegreen4", "darkgreen",
                               "darkseagreen1", "springgreen", "yellow",  "goldenrod", "sienna1", "indianred", "thistle", "firebrick", "red4",  "deeppink", "orchid1", "purple4", "peachpuff4",  "snow4", "palegoldenrod"), name = "Date")+
  scale_shape_manual(values = c(21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21,21,21,21,21,21,21,21,21,21,21,21,21,21), name = "Date")+
  scale_size_manual(values = c(2, 2, 2, 2, 2, 2, 2, 2, 2, 2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2), name = "Date")+
